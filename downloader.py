...
# Alwaysdata uchun: iloji boricha "progressive mp4" (audio+video bitta fayl) ustun bo‘lsin.
# Bu ffmpeg merge’ni kamaytiradi -> CPU/IO kamroq -> kill ehtimoli pasayadi.
QUALITY_MAP = {
    "360p":  "best[ext=mp4][height<=360][acodec!=none][vcodec!=none]/best[height<=360]/worst",
    "480p":  "best[ext=mp4][height<=480][acodec!=none][vcodec!=none]/best[height<=480]/worst",
    "720p":  "best[ext=mp4][height<=720][acodec!=none][vcodec!=none]/best[height<=720]/worst",
    # 1080+ ko‘pincha adaptive (video+audio alohida) bo‘ladi -> merge kerak bo‘lishi mumkin
...
def _build_opts(
    ...
    # Android client PO token talab qiladi -> qo‘ymaymiz
    opts.setdefault("extractor_args", {})
    opts["extractor_args"].setdefault("youtube", {})
    opts["extractor_args"]["youtube"]["player_client"] = ["tv"]

    # Alwaysdata’da kill bo‘lmasligi uchun parallelni past ushlaymiz
    opts.update({
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if est_size and est_size > max_file_bytes:
                raise FileTooLargeError(
                    f"Estimated file size {est_size} exceeds limit {max_file_bytes}. Try a lower quality."
                )
